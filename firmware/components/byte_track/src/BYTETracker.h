...
class BYTETracker {
   ...
   public:
    BYTETracker(int frame_rate = 10, int track_buffer = 15);
    BYTETracker(const bt_config_t* config);
    ~BYTETracker();

    std::vector<STrack> update(const bt_bbox_t* objects, size_t num_objects);
